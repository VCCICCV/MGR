...
use shared::{constant::Audience, global::{clear_routes, get_collected_routes, get_config}, request_id::{RequestId, RequestIdLayer}, utils::{self, add_key, api_key_middleware, protect_route, ApiKeySource, ApiKeyValidation, ComplexApiKeyConfig, SimpleApiKeyConfig, ValidatorType}};
...
async fn process_collected_routes() {
    ...
    match endpoint_service.sync_endpoints(endpoints).await {
        Ok(_) => {
            info!("Endpoints synced successfully")
        },
        Err(e) => {
            error!("Failed to sync endpoints: {:?}", e)
        },
    }
}
