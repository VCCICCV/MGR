...
use model::admin::{
    request::{
        sys_authentication::LoginInput,
        sys_authorization::{ AssignPermissionDto, AssignRouteDto },
    },
    response::sys_authentication::{ AuthOutput, UserInfoOutput, UserRoute },
};
use service::admin::{
    dto::sys_auth_dto::LoginContext,
    sys_auth_service::{ SysAuthService, TAuthService },
    sys_authorization_service::{ SysAuthorizationService, TAuthorizationService },
};
use shared::{
    constant::Audience,
    utils::ip::ClientIp,
    web::{ auth::User, error::AppError, request_id::RequestId, res::Res, validator::ValidatedForm },
};
...
impl SysAuthenticationApi {
    pub async fn login_handler(
        ...
        let address = xdb::searcher
            ::search_by_ip(client_ip.as_str())
    ...
    pub async fn get_user_info(Extension(user): Extension<User>) -> Result<
        Res<UserInfoOutput>,
        AppError
    > {
    ...
    pub async fn assign_permission(
        ...
        service.assign_permission(input.domain, input.role_id, input.permissions, enforcer).await?;
    ...
    pub async fn assign_routes(
        ...
        service.assign_routes(input.domain, input.role_id, input.route_ids).await?;
