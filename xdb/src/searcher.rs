...
fn default_detect_xdb_file() -> Result<String, Box<dyn Error>> {
    let prefix = "../".to_owned();
    for recurse in 1..4 {
        let filepath = prefix.repeat(recurse) + "/static/ip2region.xdb";
...
pub fn searcher_init(xdb_filepath: Option<String>) {
    ...
    unsafe { std::env::set_var(XDB_FILEPATH_ENV, xdb_filepath) };
    // 初始化并预热两个缓存
...
mod tests {
    ...
    fn test_match_all_ip_correct() {
        searcher_init(None);
        let mut file = File::open("/static/ip.test.txt").expect("无法打开测试数据文件，请确保static/ip.test.txt存在");
