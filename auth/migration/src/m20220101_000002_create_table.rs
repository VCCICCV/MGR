    ...
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        manager.create_table(
            Table::create()
                ...
                .col(ColumnDef::new(Order::OrderSn).string().not_null().comment("订单编号"))
                .col(ColumnDef::new(Order::UserId).big_integer().not_null().comment("用户 ID"))
                ...
                .col(ColumnDef::new(Order::DeliveryTime).date_time().not_null().comment("发货时间"))
                ...
                .col(ColumnDef::new(Order::ReceiveTime).date_time().not_null().comment("收货时间"))
                ...
                .col(ColumnDef::new(Order::PayTime).date_time().not_null().comment("支付时间"))
                .col(
                ...
                .col(ColumnDef::new(Order::CreateTime).date_time().not_null().comment("创建时间"))
                .col(ColumnDef::new(Order::UpdateTime).date_time().comment("更新时间"))
                .to_owned()
        ).await?;
        // 创建订单项表
        manager.create_table(
            Table::create()
                ...
                .col(ColumnDef::new(OrderItem::OrderId).big_integer().not_null().comment("订单 ID"))
                .col(ColumnDef::new(OrderItem::UserId).big_integer().not_null().comment("用户 ID"))

                .col(ColumnDef::new(OrderItem::OrderSn).string().not_null().comment("订单编号"))
                .col(
                ...
                .col(
                    ColumnDef::new(OrderItem::ProductSkuId)
                        .big_integer()
                        .not_null()
                        .comment("商品SKU ID")
                )
                ...
                .col(
                    ColumnDef::new(OrderItem::CreateTime).date_time().not_null().comment("创建时间")
                )
                .col(ColumnDef::new(OrderItem::UpdateTime).date_time().comment("更新时间"))
                .to_owned()
        ).await?;
        Ok(())
    }
